...
public abstract class MicroService implements Runnable {
    ...
    private final ConcurrentHashMap<Class<? extends Message>, Callback<?>> callbacksQueue;
    ...
    public MicroService(String name) {
        ...
        this.callbacksQueue= new ConcurrentHashMap<>();
    }
    ...
    public final void run() {
        messageBus.register(this); // register the microservice to messageBus
        //System.out.println("initializing "+ this.getName());
        initialize(); //using the initialize method of the specific microService
        //System.out.println("initializing "+ this.getName()+ "succeed");
        while (!terminated) {
            try{
                Message message= messageBus.awaitMessage(this);
                Callback callback= callbacksQueue.get(message.getClass());

                callback.call(message); //using call method of callback
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                terminate();
            }
        }
        messageBus.unregister(this); // unregister the microservice from messageBus
    }
